...
static void mute_fragment(struct digit_dsp_pvt *dsp, fragment_t *fragment)
{
...
static void ast_fax_detect_init(struct digit_dsp_pvt *s)
{
...
static int tone_detect(struct digit_dsp_pvt *dsp, tone_detect_state_t *s, int16_t *amp, int samples)
{
	float tone_energy;
	int i;
	int hit = 0;
...
static int dtmf_detect(struct digit_dsp_pvt *dsp, digit_detect_state_t *s, int16_t amp[], int samples, int squelch, int relax)
{
	float row_energy[4];
	float col_energy[4];
	int i;
...
static int mf_detect(struct digit_dsp_pvt *dsp, digit_detect_state_t *s, int16_t amp[],
...
static int __ast_dsp_call_progress(struct digit_dsp_pvt *dsp, short *s, int len)
{
	short samp;
	int x;
	int y;
...
static int digit_dsp_call_progress(void *pvt, struct ast_frame *inf)
{
	struct digit_dsp_pvt *dsp = (struct digit_dsp_pvt*)pvt;
...
static int __ast_dsp_silence_noise(struct digit_dsp_pvt *dsp, short *s, int len, int *totalsilence, int *totalnoise, int *frames_energy)
{
	int accum;
	int x;
	int res = 0;
...
static int digit_dsp_busydetect(void *pvt)
{
	struct digit_dsp_pvt *dsp = (struct digit_dsp_pvt*)pvt;
	int res = 0, x;
...
static int ast_dsp_silence_noise_with_energy(struct digit_dsp_pvt *dsp, struct ast_frame *f, int *total, int *frames_energy, int noise)
{
	short *s;
	int len;
	int x;
...
static int digit_dsp_silence_with_energy(void *pvt, struct ast_frame *f, int *totalsilence, int *frames_energy)
{
	struct digit_dsp_pvt *dsp = (struct digit_dsp_pvt*)pvt;
	return ast_dsp_silence_noise_with_energy(dsp, f, totalsilence, frames_energy, 0);
}

static int digit_dsp_silence(void *pvt, struct ast_frame *f, int *totalsilence)
{
	struct digit_dsp_pvt *dsp = (struct digit_dsp_pvt*)pvt;
	return ast_dsp_silence_noise_with_energy(dsp, f, totalsilence, NULL, 0);
}

static int digit_dsp_noise(void *pvt, struct ast_frame *f, int *totalnoise)
{
	struct digit_dsp_pvt *dsp = (struct digit_dsp_pvt*)pvt;
...
static struct ast_frame *digit_dsp_process(struct ast_channel *chan, void *pvt, struct ast_frame *af)
{
	struct digit_dsp_pvt *dsp = (struct digit_dsp_pvt*)pvt;
	int silence;
	int res;
	...
	if ((dsp->features & DSP_FEATURE_BUSY_DETECT) && digit_dsp_busydetect(dsp)) {
...
static void ast_dsp_prog_reset(struct digit_dsp_pvt *dsp)
{
	int max = 0;
	int x;
...
static unsigned int digit_dsp_get_sample_rate(const void *pvt)
{
	struct digit_dsp_pvt *dsp = (struct digit_dsp_pvt*)pvt;
	return dsp->sample_rate;
}

static void digit_dsp_set_features(void *pvt, int features)
{
	struct digit_dsp_pvt *dsp = (struct digit_dsp_pvt*)pvt;
...
static int digit_dsp_get_features(void *pvt)
{
	struct digit_dsp_pvt *dsp = (struct digit_dsp_pvt*)pvt;
        return (dsp->features);
}
...
static void digit_dsp_free(void *pvt)
{
	struct digit_dsp_pvt *dsp = (struct digit_dsp_pvt*)pvt;
	ast_free(dsp);
}

static void digit_dsp_set_threshold(void *pvt, int threshold)
{
	struct digit_dsp_pvt *dsp = (struct digit_dsp_pvt*)pvt;
	dsp->threshold = threshold;
}

static void digit_dsp_set_busy_count(void *pvt, int cadences)
{
	struct digit_dsp_pvt *dsp = (struct digit_dsp_pvt*)pvt;
	if (cadences < 4) {
		cadences = 4;
...
static void digit_dsp_set_busy_pattern(void *pvt, const struct ast_dsp_busy_pattern *cadence)
{
	struct digit_dsp_pvt *dsp = (struct digit_dsp_pvt*)pvt;
...
static void digit_dsp_digitreset(void *pvt)
{
	struct digit_dsp_pvt *dsp = (struct digit_dsp_pvt*)pvt;
	int i;
...
static void digit_dsp_reset(void *pvt)
{
	struct digit_dsp_pvt *dsp = (struct digit_dsp_pvt*)pvt;
	int x;
...
static int digit_dsp_set_digitmode(void *pvt, int digitmode)
{
	struct digit_dsp_pvt *dsp = (struct digit_dsp_pvt*)pvt;
	int new;
	int old;
...
static int digit_dsp_set_faxmode(void *pvt, int faxmode)
{
	struct digit_dsp_pvt *dsp = (struct digit_dsp_pvt*)pvt;
...
static int digit_dsp_set_call_progress_zone(void *pvt, char *zone)
{
	struct digit_dsp_pvt *dsp = (struct digit_dsp_pvt*)pvt;
	int x;
...
static int digit_dsp_was_muted(void *pvt)
{
	struct digit_dsp_pvt *dsp = (struct digit_dsp_pvt*)pvt;
	return (dsp->mute_fragments > 0);
}

static int digit_dsp_get_tstate(void *pvt)
{
	struct digit_dsp_pvt *dsp = (struct digit_dsp_pvt*)pvt;
	return dsp->tstate;
}

static int digit_dsp_get_tcount(void *pvt)
{
	struct digit_dsp_pvt *dsp = (struct digit_dsp_pvt*)pvt;
	return dsp->tcount;
}

struct ast_dsp_tech  digit_dsp_tech = {
	.dsp_free = digit_dsp_free,
	.dsp_get_sample_rate= digit_dsp_get_sample_rate,
	.dsp_set_threshold = digit_dsp_set_threshold,
	.dsp_set_busy_count = digit_dsp_set_busy_count,
	.dsp_set_busy_pattern = digit_dsp_set_busy_pattern,
	.dsp_call_progress = digit_dsp_call_progress,
	.dsp_set_call_progress_zone = digit_dsp_set_call_progress_zone,
	.dsp_process = digit_dsp_process,
	.dsp_silence = digit_dsp_silence,
	.dsp_silence_with_energy = digit_dsp_silence_with_energy,
	.dsp_noise = digit_dsp_noise,
	.dsp_busydetect= digit_dsp_busydetect,
	.dsp_reset = digit_dsp_reset,
	.dsp_digitreset = digit_dsp_digitreset,
	.dsp_set_features = digit_dsp_set_features,
	.dsp_get_features = digit_dsp_get_features,
	.dsp_set_digitmode = digit_dsp_set_digitmode,
	.dsp_set_faxmode = digit_dsp_set_faxmode,
	.dsp_was_muted = digit_dsp_was_muted,
	.dsp_get_tstate = digit_dsp_get_tstate,
	.dsp_get_tcount= digit_dsp_get_tcount
};
...
AST_MODULE_INFO(ASTERISK_GPL_KEY, AST_MODFLAG_GLOBAL_SYMBOLS | AST_MODFLAG_LOAD_ORDER, "DSP Detect Digits",
