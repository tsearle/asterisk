...
#define DTMF_THRESHOLD		8.0e5
#define TONE_THRESHOLD		8.0e7
...
#define DEF_RELAX_DTMF_REVERSE_TWIST	6.51	 /* 8.2dB */
#else
#define DEF_DTMF_REVERSE_TWIST		2.51	 /* 4.01dB */
#define DEF_RELAX_DTMF_REVERSE_TWIST	3.98	 /* 6.0dB */
#endif

#define DTMF_RELATIVE_PEAK_ROW	2.0     /* 8dB */
#define DTMF_RELATIVE_PEAK_COL	2.0     /* 8dB */
#define DTMF_TO_TOTAL_ENERGY	27.0
...
struct mute_dsp_pvt {
	struct ast_frame f;
	struct ast_frame * last_audio[AUDIO_DELAY];
	int threshold;
...
static void mute_fragment(struct mute_dsp_pvt *dsp, fragment_t *fragment)
{
...
static void ast_fax_detect_init(struct mute_dsp_pvt *s)
{
...
static int tone_detect(struct mute_dsp_pvt *dsp, tone_detect_state_t *s, int16_t *amp, int samples)
{
	float tone_energy;
	int i;
	int hit = 0;
...
static int dtmf_detect(struct mute_dsp_pvt *dsp, digit_detect_state_t *s, int16_t amp[], int samples, int squelch, int relax)
{
	float row_energy[4];
	float col_energy[4];
	int i;
	...
	for (sample = 0; sample < samples; sample = limit) {
		...
		if (hit && s->td.dtmf.current_sample < DTMF_GSIZE) {
 ...
 */
	...
	return hit;     /* return the debounced hit */
}

static int mf_detect(struct mute_dsp_pvt *dsp, digit_detect_state_t *s, int16_t amp[],
...
static int __ast_dsp_call_progress(struct mute_dsp_pvt *dsp, short *s, int len)
{
	short samp;
	int x;
	int y;
...
static int mute_dsp_call_progress(void *pvt, struct ast_frame *inf)
{
	struct mute_dsp_pvt *dsp = (struct mute_dsp_pvt*)pvt;
...
static int __ast_dsp_silence_noise(struct mute_dsp_pvt *dsp, short *s, int len, int *totalsilence, int *totalnoise, int *frames_energy)
{
	int accum;
	int x;
	int res = 0;
...
static int mute_dsp_busydetect(void *pvt)
{
	struct mute_dsp_pvt *dsp = (struct mute_dsp_pvt*)pvt;
	int res = 0, x;
...
static int ast_dsp_silence_noise_with_energy(struct mute_dsp_pvt *dsp, struct ast_frame *f, int *total, int *frames_energy, int noise)
{
	short *s;
	int len;
	int x;
...
static int mute_dsp_silence_with_energy(void *pvt, struct ast_frame *f, int *totalsilence, int *frames_energy)
{
	struct mute_dsp_pvt *dsp = (struct mute_dsp_pvt*)pvt;
	return ast_dsp_silence_noise_with_energy(dsp, f, totalsilence, frames_energy, 0);
}

static int mute_dsp_silence(void *pvt, struct ast_frame *f, int *totalsilence)
{
	struct mute_dsp_pvt *dsp = (struct mute_dsp_pvt*)pvt;
	return ast_dsp_silence_noise_with_energy(dsp, f, totalsilence, NULL, 0);
}

static int mute_dsp_noise(void *pvt, struct ast_frame *f, int *totalnoise)
{
	struct mute_dsp_pvt *dsp = (struct mute_dsp_pvt*)pvt;
...
static struct ast_frame *mute_dsp_process(struct ast_channel *chan, void *pvt, struct ast_frame *af)
{
	struct mute_dsp_pvt *dsp = (struct mute_dsp_pvt*)pvt;
	int silence;
	int res;
	...
	struct ast_frame * last_audio = dsp->last_audio[AUDIO_DELAY-1];
	...
	for (x = AUDIO_DELAY-1; x > 0; x--) {
		dsp->last_audio[x] = dsp->last_audio[x-1];
	}
	dsp->last_audio[0] = ast_frdup(af);
	ast_frfree(af);
	af = dsp->last_audio[0];
	...
	odata = last_audio->data.ptr;
	idata = af->data.ptr;
	olen = last_audio->datalen;
	ilen = af->datalen;
	...
	if ((dsp->features & DSP_FEATURE_SILENCE_SUPPRESS) || (dsp->features & DSP_FEATURE_BUSY_DETECT)) {
		res = __ast_dsp_silence_noise(dsp, shortdata, ilen, &silence, NULL, NULL);
	}

	if ((dsp->features & DSP_FEATURE_SILENCE_SUPPRESS) && silence) {
		memset(&dsp->f, 0, sizeof(dsp->f));
		dsp->f.frametype = AST_FRAME_NULL;
		ast_frfree(last_audio);
		return ast_frisolate(&dsp->f);
	}
	if ((dsp->features & DSP_FEATURE_BUSY_DETECT) && mute_dsp_busydetect(dsp)) {
	...
	if ((dsp->features & DSP_FEATURE_FAX_DETECT)) {
		if ((dsp->faxmode & DSP_FAXMODE_DETECT_CNG) && tone_detect(dsp, &dsp->cng_tone_state, shortdata, ilen)) {
			fax_digit = 'f';
		}

		if ((dsp->faxmode & DSP_FAXMODE_DETECT_CED) && tone_detect(dsp, &dsp->ced_tone_state, shortdata, ilen)) {
			fax_digit = 'e';
		}
	}
	...
	if (dsp->features & (DSP_FEATURE_DIGIT_DETECT | DSP_FEATURE_BUSY_DETECT)) {
		if (dsp->digitmode & DSP_DIGITMODE_MF) {
			digit = mf_detect(dsp, &dsp->digit_state, shortdata, ilen, (dsp->digitmode & DSP_DIGITMODE_NOQUELCH) == 0, (dsp->digitmode & DSP_DIGITMODE_RELAXDTMF));
		} else {
			digit = dtmf_detect(dsp, &dsp->digit_state, shortdata, ilen, (dsp->digitmode & DSP_DIGITMODE_NOQUELCH) == 0, (dsp->digitmode & DSP_DIGITMODE_RELAXDTMF));
		}
		if (dsp->mute_lag > 0) {
			writeback = 1;
			dsp->mute_lag--;
	...
	if ((dsp->features & DSP_FEATURE_CALL_PROGRESS)) {
		res = __ast_dsp_call_progress(dsp, shortdata, ilen);
		if (res) {
			switch (res) {
	...
	if (writeback || dsp->mute_fragments > 0) {
		if (ast_format_cmp(last_audio->subclass.format, ast_format_ulaw) == AST_FORMAT_CMP_EQUAL) {
			for (x = 0; x < olen; x++) {
				odata[x] = AST_LIN2MU((unsigned short) 0);
			}
		} else if (ast_format_cmp(last_audio->subclass.format, ast_format_alaw) == AST_FORMAT_CMP_EQUAL) {
			for (x = 0; x < olen; x++) {
				odata[x] = AST_LIN2A((unsigned short) 0);
			}
		}
	}
	...
	if (outf) {
		if (chan) {
			ast_queue_frame(chan, last_audio);
		}
		ast_frfree(last_audio);
...
static void ast_dsp_prog_reset(struct mute_dsp_pvt *dsp)
{
	int max = 0;
	int x;
...
static unsigned int mute_dsp_get_sample_rate(const void *pvt)
{
	struct mute_dsp_pvt *dsp = (struct mute_dsp_pvt*)pvt;
	return dsp->sample_rate;
}

static void mute_dsp_set_features(void *pvt, int features)
{
	struct mute_dsp_pvt *dsp = (struct mute_dsp_pvt*)pvt;
...
static int mute_dsp_get_features(void *pvt)
{
	struct mute_dsp_pvt *dsp = (struct mute_dsp_pvt*)pvt;
        return (dsp->features);
}
...
static void mute_dsp_free(void *pvt)
{
	struct mute_dsp_pvt *dsp = (struct mute_dsp_pvt*)pvt;
	if (dsp) {
		int x;
		for (x = 0; x < AUDIO_DELAY; x++) {
			if (dsp->last_audio[x]) {
				ast_frfree(dsp->last_audio[x]);
				dsp->last_audio[x] = NULL;
			}
		}
	}
	ast_free(dsp);
...
static void mute_dsp_set_threshold(void *pvt, int threshold)
{
	struct mute_dsp_pvt *dsp = (struct mute_dsp_pvt*)pvt;
	dsp->threshold = threshold;
}

static void mute_dsp_set_busy_count(void *pvt, int cadences)
{
	struct mute_dsp_pvt *dsp = (struct mute_dsp_pvt*)pvt;
	if (cadences < 4) {
		cadences = 4;
...
static void mute_dsp_set_busy_pattern(void *pvt, const struct ast_dsp_busy_pattern *cadence)
{
	struct mute_dsp_pvt *dsp = (struct mute_dsp_pvt*)pvt;
...
static void mute_dsp_digitreset(void *pvt)
{
	struct mute_dsp_pvt *dsp = (struct mute_dsp_pvt*)pvt;
	int i;
...
static void mute_dsp_reset(void *pvt)
{
	struct mute_dsp_pvt *dsp = (struct mute_dsp_pvt*)pvt;
	int x;
	...
	dsp->ringtimeout = 0;
	int y;
	for (y = 0; y < AUDIO_DELAY; y++) {
		if (dsp->last_audio[y])
		{
			ast_frfree(dsp->last_audio[y]);
			dsp->last_audio[y] = NULL;
		}
	}
}

static int mute_dsp_set_digitmode(void *pvt, int digitmode)
{
	struct mute_dsp_pvt *dsp = (struct mute_dsp_pvt*)pvt;
	int new;
	int old;
...
static int mute_dsp_set_faxmode(void *pvt, int faxmode)
{
	struct mute_dsp_pvt *dsp = (struct mute_dsp_pvt*)pvt;
...
static int mute_dsp_set_call_progress_zone(void *pvt, char *zone)
{
	struct mute_dsp_pvt *dsp = (struct mute_dsp_pvt*)pvt;
	int x;
...
static int mute_dsp_was_muted(void *pvt)
{
	struct mute_dsp_pvt *dsp = (struct mute_dsp_pvt*)pvt;
	return (dsp->mute_fragments > 0);
}

static int mute_dsp_get_tstate(void *pvt)
{
	struct mute_dsp_pvt *dsp = (struct mute_dsp_pvt*)pvt;
	return dsp->tstate;
}

static int mute_dsp_get_tcount(void *pvt)
{
	struct mute_dsp_pvt *dsp = (struct mute_dsp_pvt*)pvt;
	return dsp->tcount;
}

struct ast_dsp_tech  mute_dsp_tech = {
	.dsp_free = mute_dsp_free,
	.dsp_get_sample_rate= mute_dsp_get_sample_rate,
	.dsp_set_threshold = mute_dsp_set_threshold,
	.dsp_set_busy_count = mute_dsp_set_busy_count,
	.dsp_set_busy_pattern = mute_dsp_set_busy_pattern,
	.dsp_call_progress = mute_dsp_call_progress,
	.dsp_set_call_progress_zone = mute_dsp_set_call_progress_zone,
	.dsp_process = mute_dsp_process,
	.dsp_silence = mute_dsp_silence,
	.dsp_silence_with_energy = mute_dsp_silence_with_energy,
	.dsp_noise = mute_dsp_noise,
	.dsp_busydetect= mute_dsp_busydetect,
	.dsp_reset = mute_dsp_reset,
	.dsp_digitreset = mute_dsp_digitreset,
	.dsp_set_features = mute_dsp_set_features,
	.dsp_get_features = mute_dsp_get_features,
	.dsp_set_digitmode = mute_dsp_set_digitmode,
	.dsp_set_faxmode = mute_dsp_set_faxmode,
	.dsp_was_muted = mute_dsp_was_muted,
	.dsp_get_tstate = mute_dsp_get_tstate,
	.dsp_get_tcount= mute_dsp_get_tcount
};
...
{
	struct ast_dsp *dsp;
	struct mute_dsp_pvt *pvt;

	if ((dsp = ast_calloc(1, sizeof(*dsp)))) {

		if ((pvt = ast_calloc(1, sizeof(*pvt)))) {
			pvt->threshold = DEFAULT_THRESHOLD;
			pvt->features = DSP_FEATURE_SILENCE_SUPPRESS;
			pvt->busycount = DSP_HISTORY;
			pvt->digitmode = DSP_DIGITMODE_DTMF;
			pvt->faxmode = DSP_FAXMODE_DETECT_CNG;
			pvt->sample_rate = sample_rate;
			pvt->freqcount = 0;
			/* Initialize digit detector */
			ast_digit_detect_init(&pvt->digit_state, pvt->digitmode & DSP_DIGITMODE_MF, pvt->sample_rate);
			pvt->display_inband_dtmf_warning = 1;
			/* Initialize initial DSP progress detect parameters */
			ast_dsp_prog_reset(pvt);
			/* Initialize fax detector */
			ast_fax_detect_init(pvt);
		}

		dsp->dsp_pvt = pvt;
		dsp->tech = &mute_dsp_tech;
	}
	return dsp;
}
...
static int _dsp_init(int reload)
{
	...
	for (v = ast_variable_browse(cfg, "default"); v; v = v->next) {
		if (!strcasecmp(v->name, "dtmf_normal_twist")) {
...
#ifdef TEST_FRAMEWORK
AST_TEST_DEFINE(test_dsp_mute_fax_detect)
{
	struct ast_dsp *dsp;
...
#ifdef TEST_FRAMEWORK
AST_TEST_DEFINE(test_dsp_mute_dtmf_detect)
{
	int idx;
	struct ast_dsp *dsp;
	enum ast_test_result_state result;
...
{
	AST_TEST_UNREGISTER(test_dsp_mute_fax_detect);
	AST_TEST_UNREGISTER(test_dsp_mute_dtmf_detect);
...
static int load_module(void)
{
	...
	AST_TEST_REGISTER(test_dsp_mute_fax_detect);
	AST_TEST_REGISTER(test_dsp_mute_dtmf_detect);
...
AST_MODULE_INFO(ASTERISK_GPL_KEY, AST_MODFLAG_GLOBAL_SYMBOLS | AST_MODFLAG_LOAD_ORDER, "DSP Mute DTMF",
